		...
		if (d.load(true)) {
			...
			if (require != null && require.length > 0) {
				Dice.Values(require, function(v:String) {
					if (!Jotun.domain.require([v])) _setError(e, "missing.param:" + v, Filler.to(locale.missing, {name:v}));
				});
			}
			
			var x:String = Jotun.domain.params.subject;
			...
			if(e.length == 0){
				...
				if (e.length == 0) {
					...
					var un:String = n.name != null ? Reflect.field(Jotun.domain.params, n.name) : null;
					var ue:String = n.email != null ? Reflect.field(Jotun.domain.params, n.email) : null;
					if (ue != null) {
					...
					if (ms == null) {
						ms = "";
						Dice.All(Jotun.domain.params, function(p:String, v:Dynamic) { ms += "<b>" + p + "</b>: " + v;});
					}else {
						ms = Filler.to(ms, Jotun.domain.params);
					}
