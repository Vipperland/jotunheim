...
jotun_tools_Agent.prototype = {
	...
	,update: function() {
		...
		var win = window;
		this.jQuery = Object.prototype.hasOwnProperty.call(win,"$") || Object.prototype.hasOwnProperty.call(win,"jQuery");
...
var jotun_net_Broadcast = $hx_exports["Jtn"]["Broadcast"] = function() {
	...
	if(jotun_net_Broadcast.__me__ == null) {
		...
		var this1 = window;
		if(Object.prototype.hasOwnProperty.call(this1,"BroadcastChannel")) {
			this._channels = { };
		} else {
			window.addEventListener("storage",function(e) {
				var channel = e.key;
				var events = _gthis._listeners[channel];
...
jotun_net_Broadcast.prototype = {
	...
	,disconnect: function() {
		...
		jotun_utils_Dice.All(this._channels,function(p,v) {
			jotun_utils_Dice.Values(_gthis._listeners[p],function(v1) {
				_gthis.unlisten(p,v1);
			});
		});
	}
	,_openChannel: function(name) {
		var bc = this._channels[name];
		if(bc == null) {
	...
	,_proccessMsg: function(channel,data) {
		...
		jotun_utils_Dice.Values(this._listeners[channel],function(handler) {
			handler(data);
		});
	}
	,listen: function(channel,handler) {
		if(handler != null) {
			var events = this._listeners[channel];
			if(events == null) {
...
jotun_net_Loader.prototype = {
	...
	,module: function(file,data,handler) {
		this.fetch(file,{ method : "GET"},{ module : true, complete : function(r) {
			if(handler != null) {
				handler(new jotun_net_Request(r.success,r.data,r.error,file));
			}
		}});
	}
	,request: function(url,data,method,handler,headers) {
		if(method == null) {
			method = "POST";
		}
		var init = { method : method != null ? method.toUpperCase() : "POST"};
		if(data != null) {
			init.body = data;
		}
		if(headers != null) {
		...
		this.fetch(url,init,{ complete : function(r) {
			if(handler != null) {
				handler(new jotun_net_Request(r.success,r.data,r.error,url));
			}
		}});
	}
...
var jotun_data_Logger = function() {
	...
	this._events = [];
	this._events.push($bind(this,this.query));
};
...
jotun_modules_ModLib.prototype = {
	...
	,onDataRequest: function(handler) {
		if(Lambda.indexOf(this._predata,handler) == -1) {
			this._predata.push(handler);
		}
	}
	,exists: function(module) {
		return Object.prototype.hasOwnProperty.call(jotun_modules_ModLib.CACHE,module.toLowerCase());
	}
	...
	,register: function(file,content) {
		...
		if(sur.length > 1) {
					...
					if(i != -1) {
						...
						if(mod.type != null) {
							if(mod.type == "data") {
								...
								if(_gthis.onHeadBind != null) {
									_gthis.onHeadBind(file,content,mod.type);
								}
			...
			jotun_Jotun.log("\t! PARSED: " + (count - errors) + "/" + total + ", Data: " + (fdata + pdata) + " ([]:" + pdata + ",@:" + fdata + "), Errors: " + errors,4);
		} else {
...
jotun_timer_Timer.prototype = {
	...
	,setCPS: function(target) {
		...
		var interval = 1000 / this._p_target;
		var diff = interval / this._p_target;
		var count = 0;
		var channel = null;
		while(count < 60) {
...
jotun_Jotun._loadController = function(e) {
	if(!jotun_Jotun._loaded) {
		...
		jotun_gaming_actions_CommonsActionQuery.register();
		jotun_gaming_actions_CommonsRequirementQuery.register();
...
jotun_Jotun.run = function(handler) {
	...
	if(handler != null) {
		if(!jotun_Jotun._loaded && jotun_Jotun._loadPool != null) {
			jotun_Jotun._loadPool.push(handler);
		} else {
			handler();
		}
	}
...
jotun_Jotun.request = function(url,data,method,handler,headers) {
	if(method == null) {
		method = "POST";
	}
...
jotun_Jotun.module = function(file,name,data,handler) {
	if(name == null || !jotun_Jotun.resources.exists(name)) {
		jotun_Jotun.loader.module(file,data,handler);
	} else {
		handler(null);
	}
};
...
jotun_css_CSSGroup.prototype = {
	...
	,exists: function(id,content,mode) {
		...
		var self = this;
		var a = Object.prototype.hasOwnProperty.call(self,k) ? self[k].innerHTML : this.CM.innerHTML;
		var b = self["style" + k];
	...
	,add: function(css,mode) {
		...
		var self = this;
		self[p] = Std.string(self[p]) + css;
	}
	,set: function(id,style,mode) {
		var p = "style" + (mode != null ? mode.toUpperCase() : "");
		var self = this;
		self[p] = Std.string(self[p]) + this._add(id,style);
	}
	,build: function() {
...
jotun_objects_Query.prototype = $extend(jotun_objects_Resolve.prototype,{
	...
	,_changeContext: function(prop) {
		var d = this._buffer.data;
		if(!Object.prototype.hasOwnProperty.call(d,prop)) {
			this._buffer.now = [];
			d[prop] = this._buffer.now;
		} else {
			this._buffer.now = d[prop];
		}
	}
	,_exec: function(q) {
		q = this._strip(q);
			...
			if(iofd != -1) {
				...
				if(iofs == -1 || iofd < q.indexOf(" ")) {
					...
					if(Object.prototype.hasOwnProperty.call(this,key)) {
						var obj = Reflect.field(this,key);
						if(((obj) instanceof jotun_objects_Query)) {
							this._beforeProc(obj);
							obj._innerProc(subq.join("."),this._buffer);
							if(!Object.prototype.hasOwnProperty.call(this._flush,key)) {
								this._flush[key] = obj;
							}
							return;
						}
...
var jotun_dom_Display = $hx_exports["Jtn"]["Display"] = function(q,t) {
	...
	var qd = q;
	if(Object.prototype.hasOwnProperty.call(qd,"element")) {
		this.element = qd["element"];
	} else {
		this.element = q;
	}
	if(this.element != window.document) {
		...
		jotun_dom_Display._DATA["" + (this._uid == null ? "null" : Std.string(UInt.toFloat(this._uid)))] = this;
	}
...
jotun_dom_Display.fromGC = function(id) {
	return jotun_dom_Display._DATA["" + (id == null ? "null" : Std.string(UInt.toFloat(id)))];
};
...
jotun_dom_Display.prototype = $extend(jotun_objects_Query.prototype,{
	...
	,_tickMotion: function() {
		...
		if(steps.length > 0) {
			...
			if(step.delay != null && step.delay > 0) {
				...
				$bind(this,this._tickMotion).apply(this,steps);
			}
	...
	,removeLastChild: function() {
		return this.removeChildAt(this.length() - 1);
	}
	...
	,appendHtml: function(q) {
		this.element.insertAdjacentHTML("beforeend",q);
		return this;
	}
...
jotun_css_XCode._createGrid = function() {
	if(!jotun_css_XCode._inits.grid) {
		...
		jotun_css_XCode.omnibuild(".hack > .h-cel",s + "max-width:100%;max-height:100%;");
...
jotun_css_XCode.stroke = function(id,text,color,width,distance,direction,quality,blur) {
	...
	while(y < 360) {
		...
		r.push(tx.toFixed(5) + "rem" + " " + ty.toFixed(5) + "rem" + " 0 " + color);
		r.push((tx + ofx).toFixed(5) + "rem" + " " + (ty + ofy).toFixed(5) + "rem" + " 0 " + color);
	}
...
jotun_css_XCode.shadow = function(id,text,color,distance,direction,float,steps,multiplier) {
	...
	while(y < z) {
		...
		tx = cos * y;
		ty = sin * y;
		r.push((tx == 0 ? "0" : Math.round(tx) + "px") + " " + (ty == 0 ? "0" : Math.round(ty) + "px") + " 0 " + jotun_tools_Utils.colorToCss(cc,.85 - y / z * c));
	}
	y = 0;
	var oX = cos * z;
	var oY = sin * z;
...
jotun_data_BlobCache.create = function(name,data,weakRef) {
	...
	var this1 = jotun_data_BlobCache._BlobRefs;
	var value = { blob : data, url : URL.createObjectURL(data), usage : 0, weak : weakRef};
	this1.h[name] = value;
	return name;
};
jotun_data_BlobCache.setWeak = function(name) {
	var blob = jotun_data_BlobCache._BlobRefs.h[name];
	if(blob != null) {
		blob.weak = true;
		if(blob.usage == 0) {
			jotun_data_BlobCache.revoke(name);
...
jotun_data_BlobCache.revoke = function(name) {
	...
	if(blob != null) {
		...
		var _this = jotun_data_BlobCache._BlobRefs;
...
jotun_data_BlobCache.flush = function() {
	var h = jotun_data_BlobCache._BlobRefs.h;
	var blob_h = h;
	var blob_keys = Object.keys(h);
	var blob_length = blob_keys.length;
	var blob_current = 0;
	while(blob_current < blob_length) {
		var blob = blob_h[blob_keys[blob_current++]];
		URL.revokeObjectURL(blob.url);
	}
	jotun_data_BlobCache._BlobRefs.h = Object.create(null);
};
...
jotun_dom_Img.prototype = $extend(jotun_dom_Display.prototype,{
	...
	,src: function(value) {
		...
		if(value != null) {
			...
			if(src != value) {
				...
				this.events.change().call(false,true,{ src : src});
			}
			return value;
		} else {
	...
	,loadBinary: function(url) {
		var _gthis = this;
		this.abort();
		this._loader = new AbortController();
		jotun_Jotun.loader.fetch(url,{ method : "GET"},{ abort : this._loader, complete : function(r) {
			_gthis._loader = null;
			if(r.success) {
				_gthis.src(r.data.content);
				_gthis.events.progress().call(false,true,{ completed : true});
			} else {
				_gthis.events.error().call(false,true,r.error);
			}
		}});
	}
	,abort: function() {
...
jotun_dom_Select.prototype = $extend(jotun_dom_Display.prototype,{
	...
	,loadTheme: function(name) {
		this.element.innerHTML = jotun_dom_Select._themes != null ? jotun_dom_Select._themes[name] : "";
...
var jotun_gaming_actions_Resolution = function(type,data,param) {
	...
	var d = data;
	if(Object.prototype.hasOwnProperty.call(d,param)) {
		var qset = d[param];
		Reflect.deleteField(d,param);
...
jotun_gaming_actions_ActionQuery.prototype = $extend(jotun_objects_Query.prototype,{
	...
	,_JOIN: function(values) {
		return values.join(" ");
	}
	,_ASJSON: function(values) {
		return JSON.stringify(this._JOIN(values));
	}
	,_rng: null
	,seed: function(value) {
		if(this._rng == null) {
			this._rng = new jotun_math_RNG(value);
		} else {
			this._rng.set(value);
		}
		return this;
	...
	,_beforeProc: function(obj) {
		if(((obj) instanceof jotun_gaming_actions_ActionQuery)) {
			obj.invocation = this.invocation;
		}
	}
	,invocation: null
	,tracer: function() {
		...
		console.log("src/jotun/gaming/actions/ActionQuery.hx:148:","[ActionQuery:tracer] " + jotun_utils_Filler.to(messages.slice().join(" "),this.invocation));
		return this;
	}
	...
	,clamp: function(name,min,max) {
		var v = this._FLOAT(this.getDataProvider().getFloat(name),min);
		var c = Math.max(min,Math.min(max,v));
		if(v != c) {
			this.getDataProvider().setVar(name,c);
		}
		return this;
	}
	,copyto: function(source,target) {
		this.getDataProvider().setVar(target,this.getDataProvider().getVar(source));
		return this;
	}
	,swap: function(nameA,nameB) {
		var a = this.getDataProvider().getVar(nameA);
		this.getDataProvider().setVar(nameA,this.getDataProvider().getVar(nameB));
		this.getDataProvider().setVar(nameB,a);
		return this;
	}
	,wait: function(time) {
...
var jotun_gaming_actions_CommonsActionQuery = function() {
	this.dom = new jotun_gaming_actions_DomActionQuery();
	jotun_gaming_actions_ActionQuery.call(this);
};
jotun_gaming_actions_CommonsActionQuery.__name__ = "jotun.gaming.actions.CommonsActionQuery";
jotun_gaming_actions_CommonsActionQuery.register = function() {
	jotun_gaming_actions_Action.codex.add(new jotun_gaming_actions_CommonsActionQuery());
};
jotun_gaming_actions_CommonsActionQuery.__super__ = jotun_gaming_actions_ActionQuery;
jotun_gaming_actions_CommonsActionQuery.prototype = $extend(jotun_gaming_actions_ActionQuery.prototype,{
	dom: null
	,__class__: jotun_gaming_actions_CommonsActionQuery
});
...
jotun_gaming_actions_RequirementQuery.prototype = $extend(jotun_objects_Query.prototype,{
	...
	,_beforeProc: function(obj) {
		if(((obj) instanceof jotun_gaming_actions_RequirementQuery)) {
			obj.invocation = this.invocation;
		}
	}
	,invocation: null
	...
	,isnull: function(name) {
		return this._isempty(this.getDataProvider().getVar(name));
	}
...
var jotun_gaming_actions_CommonsRequirementQuery = function() {
	this.dom = new jotun_gaming_actions_DomRequirementQuery();
	jotun_gaming_actions_RequirementQuery.call(this);
};
jotun_gaming_actions_CommonsRequirementQuery.__name__ = "jotun.gaming.actions.CommonsRequirementQuery";
jotun_gaming_actions_CommonsRequirementQuery.register = function() {
	jotun_gaming_actions_Requirement.codex.add(new jotun_gaming_actions_CommonsRequirementQuery());
};
jotun_gaming_actions_CommonsRequirementQuery.__super__ = jotun_gaming_actions_RequirementQuery;
jotun_gaming_actions_CommonsRequirementQuery.prototype = $extend(jotun_gaming_actions_RequirementQuery.prototype,{
	dom: null
	,__class__: jotun_gaming_actions_CommonsRequirementQuery
});
var jotun_gaming_actions_DomActionQuery = function() {
	jotun_gaming_actions_ActionQuery.call(this);
};
jotun_gaming_actions_DomActionQuery.__name__ = "jotun.gaming.actions.DomActionQuery";
jotun_gaming_actions_DomActionQuery.__super__ = jotun_gaming_actions_ActionQuery;
jotun_gaming_actions_DomActionQuery.prototype = $extend(jotun_gaming_actions_ActionQuery.prototype,{
	current: null
	,one: function() {
		var $l=arguments.length;
		var query = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){query[$i-0]=arguments[$i];}
		this.current = jotun_Jotun.one(this._JOIN(query.slice()));
		return this;
	}
	,all: function() {
		var $l=arguments.length;
		var query = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){query[$i-0]=arguments[$i];}
		this.current = jotun_Jotun.all(this._JOIN(query.slice()));
		return this;
	}
	,css: function() {
		var $l=arguments.length;
		var classes = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){classes[$i-0]=arguments[$i];}
		if(this.current != null) {
			this.current.css(this._JOIN(classes.slice()));
		}
		return this;
	}
	,show: function() {
	...
	,mount: function(module,params) {
		if(this.current != null) {
			this.current.mount(module,params != null ? this._PARAMS(params) : this.invocation.origin);
		}
		return this;
	}
	,writeText: function() {
		...
		if(this.current != null) {
			this.current.writeText(jotun_utils_Filler.to(this._JOIN(values.slice()),this.invocation.origin));
		}
		return this;
	}
	,writeHtml: function() {
		...
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null) {
			this.current.writeHtml(jotun_utils_Filler.to(this._JOIN(values.slice()),this.invocation.origin));
		}
		return this;
	}
	,appendText: function() {
		...
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null) {
			this.current.appendText(jotun_utils_Filler.to(this._JOIN(values.slice()),this.invocation.origin));
		}
		return this;
	}
	,appendHtml: function() {
		...
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null) {
			this.current.appendHtml(jotun_utils_Filler.to(this._JOIN(values.slice()),this.invocation.origin));
		}
		return this;
	}
	,react: function() {
		...
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null) {
			this.current.react(values.length == 0 ? this.invocation.origin : JSON.parse(values.slice().join(" ")));
		}
		return this;
	}
	,attribute: function() {
		...
		var values = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null && values.length > 0) {
			this.current.attribute(JSON.parse(values.slice().join(" ")));
		}
		return this;
	}
	,style: function() {
		...
		for(var $i=0;$i<$l;++$i){values[$i-0]=arguments[$i];}
		if(this.current != null && values.length > 0) {
			this.current.style(JSON.parse(values.slice().join(" ")));
		}
		return this;
	}
	,readattr: function(attr,name) {
		if(this.current != null) {
			var v = this.current.attribute(attr);
			if(v != null) {
				this.getDataProvider().setVar(name,v);
			}
		}
	...
	,readcount: function(query,name) {
		this.getDataProvider().setVar(name,jotun_Jotun.all(query).length());
		return this;
	}
	,remove: function() {
		if(this.current != null) {
	...
	,flush: function() {
		this.current = null;
		jotun_gaming_actions_ActionQuery.prototype.flush.call(this);
	}
	,__class__: jotun_gaming_actions_DomActionQuery
});
var jotun_gaming_actions_DomRequirementQuery = function() {
	jotun_gaming_actions_RequirementQuery.call(this);
};
jotun_gaming_actions_DomRequirementQuery.__name__ = "jotun.gaming.actions.DomRequirementQuery";
jotun_gaming_actions_DomRequirementQuery.__super__ = jotun_gaming_actions_RequirementQuery;
jotun_gaming_actions_DomRequirementQuery.prototype = $extend(jotun_gaming_actions_RequirementQuery.prototype,{
	current: null
	,_el: function(query) {
		if(query.length == 0) {
			return this.current;
		} else {
			return jotun_Jotun.one(query.join(" "));
		}
	}
	,one: function() {
		var $l=arguments.length;
		var query = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){query[$i-0]=arguments[$i];}
		this.current = jotun_Jotun.one(query.slice().join(" "));
	...
	,exists: function() {
		...
		var query = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){query[$i-0]=arguments[$i];}
		return this._el(query.slice()) != null;
	}
	,visible: function() {
		var $l=arguments.length;
		var query = new Array($l>0?$l-0:0);
		for(var $i=0;$i<$l;++$i){query[$i-0]=arguments[$i];}
		var el = this._el(query.slice());
		if(el != null) {
			return el.isVisible();
		} else {
			return false;
	...
	,hasclass: function() {
		...
		var el = a.length == 1 ? this.current : jotun_Jotun.one(a.shift());
		if(el != null) {
			return el.hasCss(a[0]);
		} else {
			return false;
		}
	}
	...
	,count: function(query,rule,value) {
		return this._resolve(jotun_Jotun.all(query).length(),rule,this._INT(value,0));
	}
	,isenabled: function() {
		var $l=arguments.length;
		var query = new Array($l>0?$l-0:0);
	...
	,flush: function() {
		this.current = null;
		jotun_gaming_actions_RequirementQuery.prototype.flush.call(this);
	}
	,__class__: jotun_gaming_actions_DomRequirementQuery
});
...
jotun_gaming_actions_Requirement.load = function(id) {
	return jotun_gaming_actions_Requirement.cache[id];
};
...
jotun_gaming_actions_SpellCasting.prototype = {
	...
	,addLog: function(i,message) {
		this.log.push(jotun_tools_Utils.prefix("",this.ident + this.chain + i,"\t") + message);
	}
	,release: function(resolution,result) {
		...
		if(chain != null) {
			if(jotun_gaming_actions_SpellCasting._channelingDepth >= 32) {
				console.log("src/jotun/gaming/actions/SpellCasting.hx:104:","[SpellCasting] Max channeling depth (" + 32 + ") reached, aborting release.");
				return;
			}
...
jotun_gaming_actions_SpellGroup.prototype = {
	...
	,learn: function(action,index) {
		...
		if(action.id == null || this.getIndexOf(action) >= this._data.length) {
			this._data.splice(index,0,action);
		}
	}
...
jotun_logical_BigFlag.prototype = {
	...
	,flush: function(value,offset) {
		...
		while(o-- > 0) {
			var p = o + offset;
			this._get(p).set(p % 32,HxOverrides.substr(value,o,1) == "1");
		}
		return this;
	}
...
jotun_logical_Flag.prototype = {
	value: null
	,toggle: function(bit) {
		this.value = jotun_logical_Flag.FToggle(this.value,1 << bit);
		return this;
	}
	,put: function(bit) {
...
var jotun_math_RNG = $hx_exports["Jtn"]["RNG"] = function(seed) {
	if(seed == null) {
		seed = Math.random();
	}
	this.set(seed);
...
jotun_serial_JsonTool.customReplacer = function(a,b) {
	if(jotun_serial_JsonTool.SKIP_PREFIX != null && typeof(a) == "string" && a.substr(0,1) == jotun_serial_JsonTool.SKIP_PREFIX) {
		return null;
	}
	if(((b) instanceof jotun_logical_Flag)) {
		return b.value;
	}
	if(js_Boot.__implements(b,jotun_dom_Displayable)) {
		return b.typeOf();
	}
	return b;
};
...
jotun_signals_Signals.prototype = {
	...
	,object: null
	,_c: function(n) {
		if(!Object.prototype.hasOwnProperty.call(this._l.h,n)) {
			var this1 = this._l;
			var value = new jotun_signals_Pipe(n,this);
			this1.h[n] = value;
		}
		return this._l.h[n];
	}
	,has: function(name) {
		return Object.prototype.hasOwnProperty.call(this._l.h,name);
	}
	,get: function(name) {
		return this._c(name);
	}
...
jotun_tools_Utils.getQueryParams = function(value) {
	...
	jotun_utils_Dice.Values(value.split("&"),function(v) {
		var data = v.split("=");
		if(data.length > 1) {
			var key = decodeURIComponent(data[0].split("+").join(" "));
...
jotun_tools_Utils.createQueryParams = function(url,value,encode) {
	...
	jotun_utils_Dice.All(value,function(p,v) {
		if(typeof(v) == "string" || typeof(v) == "number" || typeof(v) == "boolean") {
			q.push(p + "=" + (encode ? encodeURIComponent(v) : v));
...
jotun_utils_Dice.Sort = function(data,key,numeric,copy) {
	...
	if(numeric) {
		...
		var cached = function(q) {
			if(!Object.prototype.hasOwnProperty.call(cache_h,q)) {
...
jotun_utils_Filler.to = function(value,data,sufix) {
	...
	if(((data) instanceof Array)) {
		jotun_utils_Dice.All(data,function(p,v) {
			var vo = v;
			vo["%0"] = p;
			r += jotun_utils_Filler._apply(sufix,value,vo);
...
jotun_utils_Reactor._commit = function(o) {
	if(o.data.__qa) {
...
jotun_utils_Reactor._commit_visibility = function(o,attr) {
...
jotun_utils_Reactor._react_fill_after = function(to,data) {
	to.all("[o-commit]").add(to).each(function(o) {
		jotun_utils_Reactor._commit(o);
		jotun_utils_Reactor._commit_visibility(o,"o-show-if");
		jotun_utils_Reactor._commit_visibility(o,"o-hide-if");
...
jotun_utils_Singularity.fetch = function(o) {
	...
	jotun_utils_Dice.Values(["onMain","onConnect","onDisconnect","onSync","onInstance","onVisibility"],function(v) {
		if(Object.prototype.hasOwnProperty.call(oc,v)) {
			jotun_utils_Singularity.signals.add(v,oc[v]);
		}
	});
};
...
jotun_utils_Table.prototype = {
	...
	,dispose: function() {
		...
		this.content = null;
		jotun_utils_Table._trash.push(this);
	}
...
jotun_data_BlobCache._BlobRefs = new haxe_ds_StringMap();
...
jotun_gaming_actions_RequirementQuery.RULE_RANDOM_DIFFERENT = "#!";
