				...
				if(p > 0) {
					...
					if(i != -1) {
						...
						if(mod.name == null) {
							...
							path += "[]";
							jotun_Jotun.log("\t\tModLib => PUSH " + mod.name,1);
						} else {
							path += "#" + mod.name;
							jotun_Jotun.log("\t\tModLib => NAME " + mod.name,1);
						}
						...
						if(mod.type != null) {
							if(mod.type == "data") {
								try {
									if(mod.name == "[]") {
										jotun_modules_ModLib.DATA.buffer.push(JSON.parse(content));
									} else {
		...
		if(Object.prototype.hasOwnProperty.call(jotun_modules_ModLib.DATA,name)) {
			...
			if(val != null) {
				try {
					...
					haxe_Log.trace("Parsing error for MOD:[" + name + "]",{ fileName : "src/jotun/modules/ModLib.hx", lineNumber : 254, className : "jotun.modules.ModLib", methodName : "getObj"});
					data = null;
				}
			}
		}
...
jotun_css_XCode.shadow = function(id,text,color,distance,direction,quality,strenght,multiplier) {
	...
	while(y < z) {
		...
		tx = cos * y;
		ty = sin * y;
		r[r.length] = (tx == 0 ? "0" : Math.round(tx) + "px") + " " + (ty == 0 ? "0" : Math.round(ty) + "px") + " 0 " + jotun_tools_Utils.colorToCss(cc,.8 - y / z * c);
	}
	y = 0;
	var oX = cos * z;
	var oY = sin * z;
...
var jotun_dom_Dialog = $hx_exports["J_dom_Dialog"] = function(q) {
	if(q == null) {
...
var jotun_gaming_actions_EventController = $hx_exports["J_EventController"] = function(data,debug) {
	this._chain = [];
	this._index = 0;
	var _gthis = this;
...
var jotun_gaming_dataform_Pulsar = $hx_exports["Pulsar"] = function(data) {
	this._open_links = { };
...
jotun_gaming_dataform_Pulsar.map = function(name,props,object,indexable,tageable) {
	...
	if(object != null) {
		object = jotun_gaming_dataform_Spark.inheritance(object);
	} else {
		object = jotun_gaming_dataform_Spark;
	}
	jotun_gaming_dataform_Pulsar._dictio[name] = { "Construct" : object == null ? jotun_gaming_dataform_Spark : object, Properties : props, Indexable : indexable, Tag : tageable};
};
...
jotun_gaming_dataform_Pulsar.prototype = {
	...
	,_encode: function(r) {
		var r1 = jotun_serial_Packager.encodeBase64(r);
		var i = 0;
		while(HxOverrides.substr(r1,r1.length - 1,1) == "=") {
			r1 = r1.substring(0,r1.length - 1);
			++i;
		}
		r1 = "#" + jotun_serial_Packager.md5Encode(r1) + i + r1;
		i = 0;
		var nr = "";
		var len = r1.length;
		while(i < len) {
			nr += HxOverrides.substr(r1,i,jotun_gaming_dataform_Pulsar.BLOCK_SIZE) + "\n";
	...
	,links: function() {
		var r = [];
		jotun_utils_Dice.Values(this._open_links,$bind(r,$arrayPush));
		return r;
	}
...
var jotun_gaming_dataform_PulsarSignals = $hx_exports["PulsarSignals"] = function() { };
jotun_gaming_dataform_PulsarSignals.__name__ = "jotun.gaming.dataform.PulsarSignals";
var jotun_gaming_dataform_Spark = $hx_exports["Spark"] = function(name) {
	jotun_gaming_dataform_SparkCore.call(this,name);
};
jotun_gaming_dataform_Spark.__name__ = "jotun.gaming.dataform.Spark";
jotun_gaming_dataform_Spark.inheritance = function(obj) {
	return jotun_tools_Utils.extendClass(obj,jotun_gaming_dataform_Spark);
};
...
jotun_gaming_dataform_Spark.prototype = $extend(jotun_gaming_dataform_SparkCore.prototype,{
	...
	,stringify: function(changes) {
		...
		if(!changes || this.isChanged()) {
			r = jotun_gaming_dataform_SparkWriter.stringify(this,this._name,changes ? this._changes : this._getProps());
		}
		jotun_utils_Dice.Values(this._inserts,function(v) {
			if(v != null) {
	...
	,parse: function(data) {
		...
		if(i[0] == this._name) {
			...
			if(data != null) {
				jotun_gaming_dataform_SparkWriter.parse(this,data,this._getProps(),false);
				return true;
			}
		}
		return false;
	...
	,merge: function(data) {
		jotun_gaming_dataform_SparkWriter.parse(this,data,this._getProps(),true);
	}
...
jotun_gaming_dataform_SparkWriter.convert = function(value,type) {
	...
	case "B":
		return value == "1";
	case "F":
		return new jotun_tools_Flag(Std.parseInt(value));
	case "I":
...
jotun_gaming_dataform_SparkWriter.stringify = function(o,n,nfo) {
	...
	jotun_utils_Dice.All(nfo,function(p,value) {
		...
		} else if(((value) instanceof jotun_tools_Flag)) {
			type = "F";
			value = value.value;
		} else if(value != null) {
			type = "O";
...
var jotun_math_JMath = $hx_exports["J_Math"] = function() { };
...
var jotun_math_RNG = $hx_exports["J_math_RNG"] = function(seed) {
	if(seed == null) {
		seed = Math.random();
	}
	this.set(seed);
...
jotun_tools_Flag.fromCard = function(hash) {
	...
	return new jotun_tools_Flag(r);
};
jotun_tools_Flag.from = function(value) {
	if(typeof(value) == "string") {
		value = Std.parseInt(value);
	}
	return new jotun_tools_Flag(value);
};
...
jotun_tools_Flag.FValue = function(hash,size,glen,glue) {
	if(glue == null) {
		glue = " ";
	}
	if(glen == null) {
...
jotun_tools_Ticker._tickAll = function() {
	...
	jotun_tools_Ticker._calcElapsed();
	if(jotun_tools_Ticker._etime >= 1) {
		jotun_utils_Dice.All(jotun_tools_Ticker._pool_low,function(p,v) {
			if(v != null) {
				v();
			}
		});
...
jotun_tools_Utils.jotunStringify = function(o,type,html) {
	if(type == null) {
		type = true;
	}
...
jotun_tools_Utils.isBetween = function(o,min,max) {
	if(o != null) {
		if(typeof(o) != "number") {
...
jotun_tools_Utils.extendClass = function(a,b) {
	haxe_Log.trace(Object,{ fileName : "src/jotun/tools/Utils.hx", lineNumber : 614, className : "jotun.tools.Utils", methodName : "extendClass", customParams : [b,b.prototype]});
...
jotun_utils_Singularity.channel = function() {
...
jotun_modules_ModLib.DATA = { buffer : []};
...
jotun_gaming_dataform_Pulsar._dictio = { };
