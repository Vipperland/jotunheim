	...
	public function execute($handler = null, $type = null, $parameters = null) {
		...
		try {
			...
			if($this->success) {
				...
				$this->errors[$this->errors->length] = new sirius_errors_Error($this->statement->errorCode(), sirius_db_Command_1($this, $handler, $p, $parameters, $type), null);
			}
			...
			$e = $_ex_;
			{
				if(Std::is($e, _hx_qtype("String"))) {
					$this->errors[$this->errors->length] = new sirius_errors_Error(0, $e, null);
				} else {
...
function sirius_db_Command_3(&$filter, &$limit, &$param, &$values, $v) {
	{
		if(sirius_utils_Dice::Match((new _hx_array(array(Reflect::field($v, $param)))), $values, 1) > 0) {
			...
			return sirius_db_Command_5($__hx__this, $filter, $limit, $param, $v, $values) && sirius_db_Command_6($__hx__this, $filter, $limit, $param, $v, $values);
		}
		return false;
	}
}
