...
class sirius_db_objects_DataTable implements sirius_db_objects_IDataTable{
	...
	public function get_description() {
		...
		if(_hx_field($this, "_description") === null) {
			...
			sirius_utils_Dice::Values($r, array(new _hx_lambda(array(&$_gthis), "sirius_db_objects_DataTable_0"), 'execute'), null);
		}
	...
	public function get_autoIncrement() {
		...
		$cmd1 = $cmd->find("AUTO_INCREMENT", "INFORMATION_SCHEMA.TABLES", sirius_db_Clause::EQUAL("TABLE_NAME", $this->_name), null, null);
	...
	public function addAll($parameters = null, $clause = null, $order = null, $limit = null) {
		...
		sirius_utils_Dice::All($parameters, array(new _hx_lambda(array(&$_gthis, &$clause, &$limit, &$order, &$parameters, &$r), "sirius_db_objects_DataTable_1"), 'execute'), null);
		return $r;
	}
	...
	public function exists($clause = null) {
		$a = $this->length($clause, sirius_db_Limit::$ONE);
		$aNeg = $a < 0;
		$bNeg = 0 < 0;
		if($aNeg !== $bNeg) {
			return $aNeg;
	...
	public function sum($field, $clause = null) {
		...
		return sirius_tools_Utils::getValidOne($tmp, 0);
	}
	public function optimize($paramaters) {
		$desc = $this->get_description();
		sirius_utils_Dice::All($paramaters, array(new _hx_lambda(array(&$desc, &$paramaters), "sirius_db_objects_DataTable_2"), 'execute'), null);
		return $paramaters;
	}
...
function sirius_db_objects_DataTable_0(&$_gthis, $v) {
	{
...
function sirius_db_objects_DataTable_1(&$_gthis, &$clause, &$limit, &$order, &$parameters, &$r, $v) {
	{
		$r1 = $r->length;
...
function sirius_db_objects_DataTable_2(&$desc, &$paramaters, $p, $v) {
	{
