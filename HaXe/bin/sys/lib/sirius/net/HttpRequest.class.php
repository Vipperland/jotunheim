...
class sirius_net_HttpRequest {
	public function __construct($url) {
		if(!isset($this->onData)) $this->onData = array(new _hx_lambda(array(&$this, &$url), "sirius_net_HttpRequest_0"), 'execute');
		if(!isset($this->onError)) $this->onError = array(new _hx_lambda(array(&$this, &$url), "sirius_net_HttpRequest_1"), 'execute');
		if(!isset($this->onStatus)) $this->onStatus = array(new _hx_lambda(array(&$this, &$url), "sirius_net_HttpRequest_2"), 'execute');
	...
	public function setHeader($header, $value) {
		$this->headers = Lambda::filter($this->headers, array(new _hx_lambda(array(&$header, &$value), "sirius_net_HttpRequest_3"), 'execute'));
	...
	public function setParameter($param, $value) {
		$this->params = Lambda::filter($this->params, array(new _hx_lambda(array(&$param, &$value), "sirius_net_HttpRequest_4"), 'execute'));
	...
	public function customRequest($post, $api, $sock = null, $method = null) {
		...
		if(_hx_field(_hx_qtype("sirius.net.HttpRequest"), "PROXY") !== null) {
			$b1->add("http://");
			$b1->add($host);
			if($port !== 80) {
				$b1->add(":");
		...
		if($this->postData !== null) {
			...
			if($post && $uri !== null) {
				if($multipart || !Lambda::exists($this->headers, array(new _hx_lambda(array(&$api, &$b1, &$boundary, &$data, &$host, &$method, &$multipart, &$port, &$portString, &$post, &$request, &$secure, &$sock, &$uri, &$url_regexp), "sirius_net_HttpRequest_6"), 'execute'))) {
					$b1->add("Content-Type: ");
					if($multipart) {
		...
		try {
			if(_hx_field(_hx_qtype("sirius.net.HttpRequest"), "PROXY") !== null) {
				$sock->connect(new sys_net_Host(sirius_net_HttpRequest::$PROXY->host), sirius_net_HttpRequest::$PROXY->port);
			} else {
	...
	static function requestUrl($url) {
		$h = new sirius_net_HttpRequest($url);
		$r = null;
...
function sirius_net_HttpRequest_0(&$__hx__this, &$url, $data) {
	{}
}
function sirius_net_HttpRequest_1(&$__hx__this, &$url, $msg) {
	{}
}
function sirius_net_HttpRequest_2(&$__hx__this, &$url, $status) {
	{}
}
function sirius_net_HttpRequest_3(&$header, &$value, $h) {
	{
		return $h->header !== $header;
	}
}
function sirius_net_HttpRequest_4(&$param, &$value, $p) {
	{
...
function sirius_net_HttpRequest_6(&$api, &$b1, &$boundary, &$data, &$host, &$method, &$multipart, &$port, &$portString, &$post, &$request, &$secure, &$sock, &$uri, &$url_regexp, $h) {
	{
		return $h->header === "Content-Type";
	}
}
function sirius_net_HttpRequest_7(&$h, &$r, &$url, $d) {
	{
		$r = $d;
	}
}
function sirius_net_HttpRequest_8(&$h, &$r, &$url, $e) {
	{
