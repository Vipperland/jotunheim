...
class sirius_net_Header {
	...
	public function setJSON($data = null, $encode = null) {
		...
		if($data !== null) {
			$data = sirius_serial_JsonTool::stringfy($data, null, "\x09");
	...
	public function writeData($data, $encode) {
		if($data !== null) {
			if($encode === true) {
				$data = sirius_serial_IOTools::encodeBase64($data);
			}
	...
	public function getClientHeaders() {
		if(_hx_field(_hx_qtype("sirius.net.Header"), "_client_headers") === null) {
			sirius_net_Header::$_client_headers = _hx_anonymous(array());
			$h = php_Lib::hashOfAssociativeArray($_SERVER);
			{
				...
				while($k->hasNext()) {
					...
					if(_hx_substr($sk, 0, 5) === "HTTP_") {
						$o = sirius_net_Header::$_client_headers;
						...
						if($tmp) {
							$o1 = sirius_net_Header::$_client_headers;
							$value1 = $h->get($k1);
							$o1->{$sk} = $value1;
							unset($value1,$o1);
						}
		...
		return sirius_net_Header::$_client_headers;
	}
