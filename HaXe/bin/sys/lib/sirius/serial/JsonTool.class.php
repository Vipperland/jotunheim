...
class sirius_serial_JsonTool {
	...
	public function fieldsString($v, $fields) {
		...
		$first = true;
		{
			...
			while($_g < $len) {
				...
				if(Std::is($f, _hx_qtype("String")) && _hx_substr($f, 0, 1) === "_") {
					continue;
				}
				if($first) {
					$this->nind++;
	...
	public function quoteUtf8($s) {
		...
		haxe_Utf8::iter($s, array(new _hx_lambda(array(&$s, &$u), "sirius_serial_JsonTool_0"), 'execute'));
		$this->buf->add("\"");
	...
	static function supressNull() {
		sirius_serial_JsonTool::$customReplacer = array(new _hx_lambda(array(), "sirius_serial_JsonTool_1"), 'execute');
	}
	static function customReplacer() { $args = func_get_args(); return call_user_func_array(self::$customReplacer, $args); }
	static $customReplacer = null;
	static function stringfy($o, $replacer = null, $space = null) {
		$printer = new sirius_serial_JsonTool(sirius_serial_JsonTool_2($o, $replacer, $space), $space);
		$printer->write("", $o);
		return $printer->buf->b;
	}
	function __toString() { return 'sirius.serial.JsonTool'; }
}
function sirius_serial_JsonTool_0(&$s, &$u, $c) {
	{
		switch($c) {
		case 92:case 34:{
			$u->addChar(92);
...
function sirius_serial_JsonTool_1($a, $b) {
	{
...
function sirius_serial_JsonTool_2(&$o, &$replacer, &$space) {
	if($replacer !== null) {
		return $replacer;
	} else {
		return (isset(sirius_serial_JsonTool::$customReplacer) ? sirius_serial_JsonTool::$customReplacer: array("sirius_serial_JsonTool", "customReplacer"));
	}
}
