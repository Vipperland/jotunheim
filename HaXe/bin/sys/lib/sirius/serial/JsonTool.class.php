...
class sirius_serial_JsonTool {
	...
	public function quoteUtf8($s) {
		...
		haxe_Utf8::iter($s, array(new _hx_lambda(array(&$u), "sirius_serial_JsonTool_0"), 'execute'));
		$this->buf->add("\"");
		$tmp = $this->buf;
		$tmp->add($u->toString());
		$this->buf->add("\"");
	...
	static function stringfy($o, $replacer = null, $space = null) {
		...
		if($replacer !== null) {
			...
			$printer = (property_exists("sirius_serial_JsonTool", "customReplacer") ? sirius_serial_JsonTool::$customReplacer: array("sirius_serial_JsonTool", "customReplacer"));
		}
		$printer1 = new sirius_serial_JsonTool($printer, $space);
		$printer1->write("", $o);
...
function sirius_serial_JsonTool_0(&$u, $c) {
	{
		switch($c) {
		case 8:{
			$u->addChar(92);
