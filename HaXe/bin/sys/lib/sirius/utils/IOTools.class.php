...
class sirius_utils_IOTools {
	public function __construct(){}
	static function encodeBase64($q) {
		if(!Std::is($q, _hx_qtype("String"))) {
			$q = sirius_utils_IOTools::jsonEncode($q, null, null);
		}
	...
	static function decodeBase64($q, $json = null) {
		...
		if($r !== null) {
			if($json && strlen($r) > 1) {
				return sirius_utils_IOTools::jsonDecode($r);
			} else {
				return $r;
			}
		} else {
	...
	static function md5Encode($o, $base64) {
		if(Std::is($o, _hx_qtype("String"))) {
			return haxe_crypto_Md5::encode($o);
		} else {
			return haxe_crypto_Md5::encode((($base64) ? sirius_utils_IOTools::encodeBase64($o) : sirius_utils_IOTools::jsonEncode($o, null, null)));
		}
	}
	function __toString() { return 'sirius.utils.IOTools'; }
}
