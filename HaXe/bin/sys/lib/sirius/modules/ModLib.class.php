...
function sirius_modules_ModLib_0(&$content, &$file, &$sur, $p, $v) {
	{
		if($p > 0) {
			...
			if($i !== -1) {
				...
				sirius_php_Sirius::log("Sirius->ModLib.build[ " . _hx_string_or_null($mod->name) . " ]", 10, 1);
				...
				if($mod->{"require"} !== null) {
					...
					sirius_php_Sirius::log("\x09Sirius->ModLib::dependencies [ FOR " . _hx_string_or_null($mod->name) . " ]", 10, 1);
				...
				sirius_php_Sirius::log("\x09Sirius->ModLib::status [ MISSING MODULE END IN " . _hx_string_or_null($file) . "(" . _hx_string_or_null(_hx_substr($v, 0, 15)) . "...) ]", 10, 3);
			}
		}
	}
}
...
function sirius_modules_ModLib_3(&$content, &$dependencies, &$end, &$file, &$i, &$mod, &$p, &$sur, &$v, $v1) {
	{
		...
		if($set === null) {
			sirius_php_Sirius::log("\x09\x09Sirius->ModLib::dependency[ MISSING " . _hx_string_or_null($v1) . " ]", 10, 2);
		} else {
			sirius_php_Sirius::log("\x09\x09Sirius->ModLib::dependency[ OK " . _hx_string_or_null($v1) . " ]", 10, 1);
