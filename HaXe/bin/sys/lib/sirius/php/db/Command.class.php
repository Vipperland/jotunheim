...
class sirius_php_db_Command implements sirius_php_db_ICommand{
	...
	public function bind($parameters) {
		...
		sirius_utils_Dice::All($parameters, array(new _hx_lambda(array(&$_g, &$isArray, &$parameters), "sirius_php_db_Command_0"), 'execute'), null);
		return $this;
	}
	...
	public function fetch($handler) {
		sirius_utils_Dice::Values($this->result, array(new _hx_lambda(array(&$handler), "sirius_php_db_Command_1"), 'execute'), null);
		return $this;
	}
	...
	public function log() {
		$q = $this->_query;
		sirius_utils_Dice::All($this->_parameters, array(new _hx_lambda(array(&$q), "sirius_php_db_Command_2"), 'execute'), null);
		return $q;
	}
	...
	function __toString() { return 'sirius.php.db.Command'; }
}
function sirius_php_db_Command_0(&$_g, &$isArray, &$parameters, $p, $v) {
	{
		if($isArray) {
...
function sirius_php_db_Command_1(&$handler, $v) {
	{
		call_user_func_array($handler, array(new sirius_data_DataSet($v)));
	}
}
function sirius_php_db_Command_2(&$q, $p, $v) {
	{
