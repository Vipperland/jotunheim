...
class sirius_php_file_FileCollection {
	...
	public function getFileNames() {
		...
		sirius_utils_Dice::Values($this->{"list"}, array(new _hx_lambda(array(&$r), "sirius_php_file_FileCollection_0"), 'execute'), null);
		return $r;
	}
	public function getByType($type = null) {
		...
		sirius_utils_Dice::Values($this->{"list"}, array(new _hx_lambda(array(&$r, &$type), "sirius_php_file_FileCollection_1"), 'execute'), null);
		return $r;
	}
...
function sirius_php_file_FileCollection_0(&$r, $v) {
	{
		if($v->type === "image") {
			$r[$r->length] = $v->output;
		}
...
function sirius_php_file_FileCollection_1(&$r, &$type, $v) {
	{
		if($type === "*") {
			if($v->type === $type) {
