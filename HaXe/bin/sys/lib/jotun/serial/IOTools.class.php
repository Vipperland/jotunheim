<?php

// Generated by Haxe 3.4.7
class jotun_serial_IOTools {
	public function __construct(){}
	static function encodeBase64($q) {
		if(!Std::is($q, _hx_qtype("String"))) {
			$q = jotun_serial_IOTools::jsonEncode($q, null, null);
		}
		return haxe_crypto_Base64::encode(haxe_io_Bytes::ofString($q), null);
	}
	static function decodeBase64($q, $json = null) {
		$r = null;
		try {
			$r = haxe_crypto_Base64::decode($q, null)->toString();
		}catch(Exception $__hx__e) {
			$_ex_ = ($__hx__e instanceof HException) && $__hx__e->getCode() == null ? $__hx__e->e : $__hx__e;
			$e = $_ex_;
			{}
		}
		if($r !== null) {
			$tmp = null;
			if($json) {
				$tmp = strlen($r) > 1;
			} else {
				$tmp = false;
			}
			if($tmp) {
				return jotun_serial_IOTools::jsonDecode($r);
			} else {
				return $r;
			}
		} else {
			return null;
		}
	}
	static function jsonEncode($o, $rep = null, $space = null) {
		return jotun_serial_JsonTool::stringify($o, $rep, $space);
	}
	static function jsonDecode($q) {
		return haxe_Json::phpJsonDecode($q);
	}
	static function md5Encode($o, $base64 = null) {
		if(Std::is($o, _hx_qtype("String"))) {
			return haxe_crypto_Md5::encode($o);
		} else {
			$tmp = null;
			if($base64) {
				$tmp = jotun_serial_IOTools::encodeBase64($o);
			} else {
				$tmp = jotun_serial_IOTools::jsonEncode($o, null, null);
			}
			return haxe_crypto_Md5::encode($tmp);
		}
	}
	function __toString() { return 'jotun.serial.IOTools'; }
}
