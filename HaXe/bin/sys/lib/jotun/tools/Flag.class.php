...
class jotun_tools_Flag {
	...
	public function putAll($bits) {
		$_gthis = $this;
		jotun_utils_Dice::Values($bits, array(new _hx_lambda(array(&$_gthis), "jotun_tools_Flag_0"), 'execute'), null);
		return $this;
	}
	public function dropAll($bits) {
		$_gthis = $this;
		jotun_utils_Dice::Values($bits, array(new _hx_lambda(array(&$_gthis), "jotun_tools_Flag_1"), 'execute'), null);
		return $this;
	}
	public function testAll($bits) {
		$_gthis = $this;
		return jotun_utils_Dice::Values($bits, array(new _hx_lambda(array(&$_gthis), "jotun_tools_Flag_2"), 'execute'), null)->completed;
	}
	public function testAny($bits, $min = null) {
		...
		$_gthis = $this;
		return !jotun_utils_Dice::Values($bits, array(new _hx_lambda(array(&$_gthis, &$min), "jotun_tools_Flag_3"), 'execute'), null)->completed;
	}
...
function jotun_tools_Flag_0(&$_gthis, $v) {
	{
		$_gthis->put(1 << $v);
	}
}
function jotun_tools_Flag_1(&$_gthis, $v) {
	{
		$_gthis->drop(1 << $v);
	}
}
function jotun_tools_Flag_2(&$_gthis, $v) {
	{
		return !$_gthis->test($v);
	}
}
function jotun_tools_Flag_3(&$_gthis, &$min, $v) {
	{
		if($_gthis->test($v)) {
			$min = $min - 1;
		}
