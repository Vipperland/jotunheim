...
class Domain implements IDomain {
	...
	public function _getMultipartKey () {
		#src/jotun/net/Domain.hx:208: lines 208-215
		if ($this->isRequestMethod("POST")) {
			#src/jotun/net/Domain.hx:209: characters 5-54
			$a = $_POST;
			#src/jotun/net/Domain.hx:210: characters 5-46
			$post = Lib::hashOfAssociativeArray($a);
			#src/jotun/net/Domain.hx:211: characters 17-28
			$data = \array_values(\array_map("strval", \array_keys($post->data)));
			$_g_current = 0;
			$_g_length = \count($data);
			$_g_data = $data;
			#src/jotun/net/Domain.hx:211: lines 211-214
			while ($_g_current < $_g_length) {
				$key = $_g_data[$_g_current++];
				#src/jotun/net/Domain.hx:212: lines 212-213
				if (HxString::indexOf($key, "Content-Disposition:_form-data;_name") !== -1) {
					#src/jotun/net/Domain.hx:213: characters 7-17
					return $key;
				}
			}
		}
		#src/jotun/net/Domain.hx:216: characters 4-15
		return null;
	}
	...
	public function _getParams () {
		...
		#src/jotun/net/Domain.hx:164: characters 5-43
	...
	public function _getRawData ($boundary, $data = null) {
		...
		if ($data === null) {
			#src/jotun/net/Domain.hx:176: characters 5-14
			$data = new HxAnon();
		}
		#src/jotun/net/Domain.hx:178: characters 4-78
		$content = file_get_contents('php://input');
		#src/jotun/net/Domain.hx:179: characters 4-55
		$result = HxString::split($content, $boundary);
		#src/jotun/net/Domain.hx:180: lines 180-201
		Dice::Values($result, function ($v) use (&$data) {
			#src/jotun/net/Domain.hx:181: lines 181-183
			if (($v === null) || (mb_strlen($v) === 0)) {
				#src/jotun/net/Domain.hx:182: characters 6-12
				return;
			}
			#src/jotun/net/Domain.hx:184: lines 184-200
			if (HxString::indexOf($v, "Content-Disposition: form-data;") < 30) {
				#src/jotun/net/Domain.hx:185: characters 6-102
				$point = HxString::split((HxString::split($v, "Content-Disposition: form-data; name=")->arr[1] ?? null), "\x0D\x0A\x0D\x0A");
				#src/jotun/net/Domain.hx:186: characters 6-55
				$param = HxString::split(($point->arr[0] ?? null), "\"")->join("");
				#src/jotun/net/Domain.hx:187: lines 187-199
				if (HxString::indexOf($param, "Content-Type:") === -1) {
					#src/jotun/net/Domain.hx:188: characters 7-52
					$value = (HxString::split(($point->arr[1] ?? null), "\x0D\x0A")->arr[0] ?? null);
					#src/jotun/net/Domain.hx:189: lines 189-198
					if ((mb_strlen($param) > 0) && (mb_strlen($value) > 0) && ($value !== "null")) {
						#src/jotun/net/Domain.hx:190: lines 190-197
						if (HxString::indexOf($param, "[]") === -1) {
							#src/jotun/net/Domain.hx:191: characters 9-45
							\Reflect::setField($data, $param, $value);
						} else {
							#src/jotun/net/Domain.hx:193: lines 193-195
							if (!\Reflect::hasField($data, $param)) {
								#src/jotun/net/Domain.hx:194: characters 10-43
								\Reflect::setField($data, $param, new \Array_hx());
							}
							#src/jotun/net/Domain.hx:196: characters 9-47
		...
		#src/jotun/net/Domain.hx:203: characters 4-15
		return $data;
	}
	...
	public function getInput () {
		#src/jotun/net/Domain.hx:258: characters 4-75
		$data = file_get_contents('php://input');
		#src/jotun/net/Domain.hx:259: characters 11-56
		if (($data !== null) && (mb_strlen($data) > 0)) {
			#src/jotun/net/Domain.hx:259: characters 45-49
			return $data;
		} else {
			#src/jotun/net/Domain.hx:259: characters 52-56
			return null;
		}
	}
	...
	public function paramAsArray ($q, $split = ",") {
		#src/jotun/net/Domain.hx:276: lines 276-279
		if ($split === null) {
			$split = ",";
		}
		#src/jotun/net/Domain.hx:277: characters 3-31
		$q = \Reflect::field($this->params, $q);
		#src/jotun/net/Domain.hx:278: characters 10-41
		if ($q !== null) {
			#src/jotun/net/Domain.hx:278: characters 22-36
			return HxString::split($q, $split);
		} else {
			#src/jotun/net/Domain.hx:278: characters 39-41
			return new \Array_hx();
		}
	}
	...
	public function paramAsBool ($q) {
		#src/jotun/net/Domain.hx:265: characters 3-49
	...
	public function paramAsFloat ($q) {
		#src/jotun/net/Domain.hx:273: characters 3-50
	...
	public function paramAsInt ($q) {
		#src/jotun/net/Domain.hx:269: characters 3-48
	...
	public function paramAsObject ($q) {
		#src/jotun/net/Domain.hx:282: characters 3-31
		$q = \Reflect::field($this->params, $q);
		#src/jotun/net/Domain.hx:283: characters 10-42
		if ($q !== null) {
			#src/jotun/net/Domain.hx:283: characters 24-37
			return Json::phpJsonDecode($q);
		} else {
			#src/jotun/net/Domain.hx:283: characters 40-42
			return new HxAnon();
		}
	}
	...
	public function parseFiles ($onPart, $onData) {
		#src/jotun/net/Domain.hx:220: characters 4-52
		$files = $_FILES;
		#src/jotun/net/Domain.hx:221: lines 221-223
		if (!isset($files)) {
			#src/jotun/net/Domain.hx:222: characters 5-11
			return;
		}
		#src/jotun/net/Domain.hx:224: characters 4-66
		$keys = array_keys($files);
		#src/jotun/net/Domain.hx:225: characters 4-54
		$parts = \Array_hx::wrap($keys);
		#src/jotun/net/Domain.hx:226: lines 226-253
		$_g = 0;
		while ($_g < $parts->length) {
			#src/jotun/net/Domain.hx:226: characters 8-12
			$part = ($parts->arr[$_g] ?? null);
			#src/jotun/net/Domain.hx:226: lines 226-253
			++$_g;
			#src/jotun/net/Domain.hx:227: characters 5-61
			$info = $_FILES[$part];
			#src/jotun/net/Domain.hx:228: characters 5-49
			$tmp = $info["tmp_name"];
			#src/jotun/net/Domain.hx:229: characters 5-46
			$file = $info["name"];
			#src/jotun/net/Domain.hx:230: characters 5-43
			$err = $info["error"];
			#src/jotun/net/Domain.hx:231: lines 231-241
			if ($err > 0) {
				#src/jotun/net/Domain.hx:232: lines 232-240
				if ($err === 1) {
					...
					#src/jotun/net/Domain.hx:236: characters 15-23
					continue;
				} else if ($err === 6) {
			...
			#src/jotun/net/Domain.hx:242: characters 5-23
			$onPart($part, $file);
			#src/jotun/net/Domain.hx:243: lines 243-252
			if ("" !== $file) {
				#src/jotun/net/Domain.hx:244: characters 6-59
				$h = fopen($tmp,"r");
				#src/jotun/net/Domain.hx:245: characters 6-23
				$bsize = 8192;
				#src/jotun/net/Domain.hx:246: lines 246-250
				while (!feof($h)) {
					#src/jotun/net/Domain.hx:247: characters 7-71
					$buf = fread($h,$bsize);
					#src/jotun/net/Domain.hx:248: characters 7-61
					$size = strlen($buf);
					#src/jotun/net/Domain.hx:249: characters 14-33
					$tmp1 = \strlen($buf);
					#src/jotun/net/Domain.hx:249: characters 7-43
					$onData(new Bytes($tmp1, new Container($buf)), 0, $size);
				}
				#src/jotun/net/Domain.hx:251: characters 6-40
				fclose($h);
			}
		}
	}
