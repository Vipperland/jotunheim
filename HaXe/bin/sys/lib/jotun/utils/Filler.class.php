...
class jotun_utils_Filler {
	...
	static function _apply($path, $content, $data) {
		if($data === null) {
			...
			if($tmp) {
				...
				jotun_utils_Dice::All($data, array(new _hx_lambda(array(&$content, &$path), "jotun_utils_Filler_0"), 'execute'), null);
			}
		}
		return $content;
	}
	static function to($value, $data, $sufix = null) {
		...
		if(Std::is($data, _hx_qtype("Array"))) {
			jotun_utils_Dice::All($data, array(new _hx_lambda(array(&$r, &$sufix, &$value), "jotun_utils_Filler_1"), 'execute'), null);
		} else {
	...
	function __toString() { return 'jotun.utils.Filler'; }
}
function jotun_utils_Filler_0(&$content, &$path, $p, $v) {
	{
		$content = jotun_utils_Filler::_apply(_hx_string_or_null($path) . _hx_string_or_null($p), $content, $v);
	}
}
function jotun_utils_Filler_1(&$r, &$sufix, &$value, $p, $v) {
	{
		$v->{"%0"} = $p;
		$r1 = jotun_utils_Filler::_apply($sufix, $value, $v);
