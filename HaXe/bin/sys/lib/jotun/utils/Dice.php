...
class Dice {
	...
	public static function List ($data, $a = 0, $b = null) {
		...
		if (($aNeg !== $bNeg ? $aNeg : $b > $a)) {
			...
			while (true) {
				...
				#src/jotun/utils/Dice.hx:397: characters 15-22
				$data1 = ($data->arr[$a] ?? null);
				#src/jotun/utils/Dice.hx:397: characters 5-23
				$copy->arr[$copy->length++] = $data1;
			...
			if (($aNeg !== $bNeg ? $aNeg : $a > $b)) {
				...
				while (true) {
					...
					if (($aNeg1 !== $bNeg1 ? $aNeg1 : $len > $a)) {
						#src/jotun/utils/Dice.hx:403: characters 16-23
						$data1 = ($data->arr[$a] ?? null);
						#src/jotun/utils/Dice.hx:403: characters 6-24
						$copy->arr[$copy->length++] = $data1;
					}
		...
		if (($values instanceof \Closure) || ($values instanceof HxClosure)) {
			...
			Dice::Values($values, function ($v) use (&$table, &$r, &$values, &$limit) {
				...
				if ($values($v) === true) {
					...
					if ($table->remove($v)) {
						#src/jotun/utils/Dice.hx:255: characters 7-16
						$r->arr[$r->length++] = $v;
			...
			Dice::Values($values, function ($v) use (&$table, &$r) {
				...
				if ($table->remove($v)) {
					#src/jotun/utils/Dice.hx:269: characters 6-15
					$r->arr[$r->length++] = $v;
				}
			});
		}
	...
	public static function Sort ($data, $key = null, $numeric = false, $copy = false) {
		...
		if ($numeric) {
			...
			#src/jotun/utils/Dice.hx:365: characters 35-44
			$cache_data = [];
			#src/jotun/utils/Dice.hx:366: lines 366-371
			$cached = function ($q) use (&$cache_data) {
				#src/jotun/utils/Dice.hx:367: lines 367-369
				if (!\array_key_exists($q, $cache_data)) {
					#src/jotun/utils/Dice.hx:368: characters 6-32
					$value = Utils::tag($q);
					$cache_data[$q] = $value;
				}
				#src/jotun/utils/Dice.hx:370: characters 5-24
				return ($cache_data[$q] ?? null);
			};
