...
class Action extends Resolution {
	...
	public static function _log ($evt, $context, $success, $score, $position) {
		#src/jotun/gaming/actions/Action.hx:90: characters 76-126
		$tmp = (Utils::isValid($evt->id) ? "#{" . ($evt->id??'null') . "} " : "");
		#src/jotun/gaming/actions/Action.hx:90: characters 3-213
		$context->addLog(0, "↑ " . ((($success ? "SUCCESS" : "FAILED"))??'null') . " ACTION " . ($tmp??'null') . "[" . ($position??'null') . "] score:" . ($score??'null') . "/" . ($evt->target??'null') . " queries:" . ($evt->length()??'null'));
	}
	...
	public function run ($context, $position) {
		#src/jotun/gaming/actions/Action.hx:61: characters 3-12
		$this->connect();
		#src/jotun/gaming/actions/Action.hx:63: characters 3-26
		$resolution = 0;
		#src/jotun/gaming/actions/Action.hx:64: characters 3-18
		++$context->ident;
		#src/jotun/gaming/actions/Action.hx:65: lines 65-73
		Dice::All($this->requirements, function ($p, $r) use (&$context, &$resolution) {
			#src/jotun/gaming/actions/Action.hx:66: characters 4-43
			$result = $r->verify($context, $p);
			#src/jotun/gaming/actions/Action.hx:67: lines 67-71
			if ($result) {
				#src/jotun/gaming/actions/Action.hx:68: characters 5-17
				$resolution += 1;
			} else {
				#src/jotun/gaming/actions/Action.hx:70: characters 5-17
				$resolution -= 1;
			}
			#src/jotun/gaming/actions/Action.hx:72: characters 4-32
		...
		if ($success) {
			#src/jotun/gaming/actions/Action.hx:81: characters 4-32
