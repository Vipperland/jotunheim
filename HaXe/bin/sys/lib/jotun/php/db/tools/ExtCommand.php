...
class ExtCommand extends CommandCore implements IExtCommand {
	...
	public function __construct ($statement, $query, $parameters, $errors, $log) {
		#src/jotun/php/db/tools/ExtCommand.hx:18: lines 18-95
	...
	public function execute ($handler = null, $type = null, $parameters = null, $contructArgs = null) {
		#src/jotun/php/db/tools/ExtCommand.hx:21: lines 21-62
		if ($this->statement !== null) {
			#src/jotun/php/db/tools/ExtCommand.hx:22: characters 4-29
			$p = null;
			#src/jotun/php/db/tools/ExtCommand.hx:23: lines 23-27
			if ($parameters !== null) {
				#src/jotun/php/db/tools/ExtCommand.hx:24: characters 5-35
				$p = $parameters->arr;
			} else if ($this->_parameters !== null) {
				#src/jotun/php/db/tools/ExtCommand.hx:26: characters 5-36
				$p = $this->_parameters->arr;
			}
			#src/jotun/php/db/tools/ExtCommand.hx:28: lines 28-56
			try {
				#src/jotun/php/db/tools/ExtCommand.hx:29: lines 29-35
				if ($type !== null) {
					#src/jotun/php/db/tools/ExtCommand.hx:30: lines 30-32
					if (!(is_float($type) || is_int($type)) && !is_string($type)) {
						#src/jotun/php/db/tools/ExtCommand.hx:31: characters 7-59
						$type = HxString::split(\Type::getClassName($type), ".")->join("\\");
					}
				} else {
					#src/jotun/php/db/tools/ExtCommand.hx:34: characters 6-25
					$type = "\\stdClass";
				}
				#src/jotun/php/db/tools/ExtCommand.hx:36: characters 5-35
				$this->success = $this->statement->execute($p);
				#src/jotun/php/db/tools/ExtCommand.hx:37: lines 37-48
				if ($this->success) {
					...
					while (true) {
						...
						#src/jotun/php/db/tools/ExtCommand.hx:41: characters 7-23
						$_this = $this->result;
					...
					#src/jotun/php/db/tools/ExtCommand.hx:47: characters 6-91
					$_this = $this->get_errors();
					#src/jotun/php/db/tools/ExtCommand.hx:47: characters 28-49
					$x = $this->statement->errorCode();
					#src/jotun/php/db/tools/ExtCommand.hx:47: characters 6-91
					$x1 = new Error($x, \Array_hx::wrap($this->statement->errorInfo()));
				...
				if (is_string($e)) {
					#src/jotun/php/db/tools/ExtCommand.hx:52: characters 6-34
					$_this = $this->get_errors();
					$x = new Error(0, $e);
					$_this->arr[$_this->length++] = $x;
				} else {
					#src/jotun/php/db/tools/ExtCommand.hx:54: characters 6-57
					$_this = $this->get_errors();
					#src/jotun/php/db/tools/ExtCommand.hx:54: characters 28-39
					$x = $e->getCode();
					#src/jotun/php/db/tools/ExtCommand.hx:54: characters 6-57
			...
			#src/jotun/php/db/tools/ExtCommand.hx:61: characters 4-72
			$_this = $this->get_errors();
			$x = new Error(0, "A connection with database is required.");
			$_this->arr[$_this->length++] = $x;
		}
	...
	public function find ($param, $values, $limit = 0) {
		...
		Dice::Values($this->result, function ($v) use (&$filter, &$param, &$values, &$limit) {
			...
			if (Dice::Match(\Array_hx::wrap([\Reflect::field($v, $param)]), $values, 1) > 0) {
				#src/jotun/php/db/tools/ExtCommand.hx:75: characters 5-19
				$filter->arr[$filter->length++] = $v;
	...
	public function length ($prop = "COUNT(*)") {
		...
		if (($this->result !== null) && ($this->result->length > 0)) {
			#src/jotun/php/db/tools/ExtCommand.hx:85: characters 4-51
			$r0 = ($this->result->arr[0] ?? null);
			#src/jotun/php/db/tools/ExtCommand.hx:86: lines 86-90
			if (\Reflect::hasField($r0, $prop)) {
				#src/jotun/php/db/tools/ExtCommand.hx:87: characters 5-38
