<?php
/**
 * Generated by Haxe 4.3.7
 */

namespace jotun\php\db;

use \php\_Boot\HxAnon;
use \php\Boot;
use \jotun\utils\Dice;

/**
 * ...
 * @author Rafael Moreira
 */
class Clause {
	/**
	 * @var string
	 */
	public $_joiner;
	/**
	 * @var mixed
	 * Inner conditions
	 */
	public $conditions;

	/**
	 * Group by entries by (A && B && ... && N)
	 * @param	conditions
	 * @return
	 * 
	 * @param mixed[]|\Array_hx $conditions
	 * 
	 * @return Clause
	 */
	public static function AND ($conditions) {
		#src/jotun/php/db/Clause.hx:42: characters 3-41
		return new Clause($conditions, " AND ");
	}

	/**
	 * IF A>=B && A<=C
	 * @param	param
	 * @param	from
	 * @param	to
	 * @return
	 * 
	 * @param string $param
	 * @param int $from
	 * @param int $to
	 * 
	 * @return mixed
	 */
	public static function BETWEEN ($param, $from, $to) {
		#src/jotun/php/db/Clause.hx:235: characters 3-76
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} BETWEEN ? AND ?",
			"value" => \Array_hx::wrap([
				$from,
				$to,
			]),
		]);
	}

	/**
	 * Add a custom condition
	 * @param	condition
	 * @return
	 * 
	 * @param string $condition
	 * 
	 * @return mixed
	 */
	public static function CUSTOM ($condition) {
		#src/jotun/php/db/Clause.hx:100: characters 3-66
		return new HxAnon([
			"param" => "",
			"condition" => $condition,
			"value" => null,
			"skip" => true,
		]);
	}

	/**
	 * IF A=B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function DIFFERENT ($param, $value) {
		#src/jotun/php/db/Clause.hx:91: characters 3-60
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}!=?",
			"value" => $value,
		]);
	}

	/**
	 * IF A=B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function EQUAL ($param, $value) {
		#src/jotun/php/db/Clause.hx:81: characters 3-59
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}=?",
			"value" => $value,
		]);
	}

	/**
	 * IF A=0
	 * @param	param
	 * @return
	 * 
	 * @param string $param
	 * 
	 * @return mixed
	 */
	public static function FALSE ($param) {
		#src/jotun/php/db/Clause.hx:180: characters 3-59
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}=?",
			"value" => false,
		]);
	}

	/**
	 * IF A&B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param int $value
	 * 
	 * @return mixed
	 */
	public static function FLAG ($param, $value) {
		#src/jotun/php/db/Clause.hx:269: characters 3-61
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} & ?",
			"value" => $value,
		]);
	}

	/**
	 * IF A|1 AND A|2 AND A|...N [NEED ALL]
	 * @param	param
	 * @param	flags
	 * @param	any		IF A|1 OR A|2 OR A|...N [NEED ANY]
	 * @return
	 * 
	 * @param string $param
	 * @param int[]|\Array_hx $flags
	 * @param bool $any
	 * 
	 * @return Clause
	 */
	public static function FLAGS ($param, $flags, $any = false) {
		#src/jotun/php/db/Clause.hx:256: lines 256-260
		if ($any === null) {
			$any = false;
		}
		#src/jotun/php/db/Clause.hx:257: characters 3-29
		$a = new \Array_hx();
		#src/jotun/php/db/Clause.hx:258: characters 3-79
		Dice::Values($flags, function ($v) use (&$param, &$a) {
			#src/jotun/php/db/Clause.hx:258: characters 46-75
			$x = Clause::FLAG($param, $v);
			$a->arr[$a->length++] = $x;
		});
		#src/jotun/php/db/Clause.hx:259: characters 10-44
		if ($any) {
			#src/jotun/php/db/Clause.hx:259: characters 16-28
			return Clause::OR($a);
		} else {
			#src/jotun/php/db/Clause.hx:259: characters 31-44
			return Clause::AND($a);
		}
	}

	/**
	 * IF A&~B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param int $value
	 * 
	 * @return mixed
	 */
	public static function FLAG_NOT ($param, $value) {
		#src/jotun/php/db/Clause.hx:279: characters 3-62
		return new HxAnon([
			"param" => $param,
			"condition" => "~{{p}} & ?",
			"value" => $value,
		]);
	}

	/**
	 * IF A>B
	 * @param	param
	 * @param	value
	 * @param	or
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function GREATER ($param, $value) {
		#src/jotun/php/db/Clause.hx:213: characters 3-59
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}>?",
			"value" => $value,
		]);
	}

	/**
	 * IF A>B
	 * @param	param
	 * @param	value
	 * @param	or
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function GREATER_OR ($param, $value) {
		#src/jotun/php/db/Clause.hx:224: characters 3-60
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}>=?",
			"value" => $value,
		]);
	}

	/**
	 * IF id=A
	 * @param	value
	 * @return
	 * 
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function ID ($value) {
		#src/jotun/php/db/Clause.hx:71: characters 3-58
		return new HxAnon([
			"param" => "id",
			"condition" => "{{p}}=?",
			"value" => $value,
		]);
	}

	/**
	 * IF A In (B,B,...)
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $values
	 * 
	 * @return mixed
	 */
	public static function IN ($param, $values) {
		#src/jotun/php/db/Clause.hx:130: lines 130-136
		if (($values instanceof \Array_hx)) {
			#src/jotun/php/db/Clause.hx:131: characters 4-29
			$q = new \Array_hx();
			#src/jotun/php/db/Clause.hx:132: characters 4-73
			Dice::All($values, function ($p, $v) use (&$q) {
				#src/jotun/php/db/Clause.hx:132: characters 58-69
				$q->arr[$q->length++] = "?";
			});
			#src/jotun/php/db/Clause.hx:133: characters 4-84
			return new HxAnon([
				"param" => $param,
				"condition" => "{{p}} IN (" . ($q->join(",")??'null') . ")",
				"value" => $values,
			]);
		} else {
			#src/jotun/php/db/Clause.hx:135: characters 4-66
			return new HxAnon([
				"param" => $param,
				"condition" => "{{p}} IN (?)",
				"value" => $values,
			]);
		}
	}

	/**
	 * If A contains B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * 
	 * @return mixed
	 */
	public static function IS_NULL ($param) {
		#src/jotun/php/db/Clause.hx:110: characters 3-75
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} IS NULL",
			"value" => null,
			"skip" => true,
		]);
	}

	/**
	 * IF A<[=]B
	 * @param	param
	 * @param	value
	 * @param	or
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function LESS ($param, $value) {
		#src/jotun/php/db/Clause.hx:191: characters 3-59
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}<?",
			"value" => $value,
		]);
	}

	/**
	 * IF A<=B
	 * @param	param
	 * @param	value
	 * @param	or
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function LESS_OR ($param, $value) {
		#src/jotun/php/db/Clause.hx:202: characters 3-60
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}<=?",
			"value" => $value,
		]);
	}

	/**
	 * If A contains B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function LIKE ($param, $value) {
		#src/jotun/php/db/Clause.hx:52: characters 3-64
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} LIKE ?",
			"value" => $value,
		]);
	}

	/**
	 * IF A<=B && A>=C
	 * @param	param
	 * @param	from
	 * @param	to
	 * @return
	 * 
	 * @param string $param
	 * @param int $from
	 * @param int $to
	 * 
	 * @return mixed
	 */
	public static function NOT_BETWEEN ($param, $from, $to) {
		#src/jotun/php/db/Clause.hx:246: characters 3-80
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} NOT BETWEEN ? AND ?",
			"value" => \Array_hx::wrap([
				$from,
				$to,
			]),
		]);
	}

	/**
	 * IF A NOT IN (B,B,...))
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $values
	 * 
	 * @return mixed
	 */
	public static function NOT_IN ($param, $values) {
		#src/jotun/php/db/Clause.hx:146: lines 146-152
		if (($values instanceof \Array_hx)) {
			#src/jotun/php/db/Clause.hx:147: characters 4-29
			$q = new \Array_hx();
			#src/jotun/php/db/Clause.hx:148: characters 4-73
			Dice::All($values, function ($p, $v) use (&$q) {
				#src/jotun/php/db/Clause.hx:148: characters 58-69
				$q->arr[$q->length++] = "?";
			});
			#src/jotun/php/db/Clause.hx:149: characters 4-88
			return new HxAnon([
				"param" => $param,
				"condition" => "{{p}} NOT IN (" . ($q->join(",")??'null') . ")",
				"value" => $values,
			]);
		} else {
			#src/jotun/php/db/Clause.hx:151: characters 4-70
			return new HxAnon([
				"param" => $param,
				"condition" => "{{p}} NOT IN (?)",
				"value" => $values,
			]);
		}
	}

	/**
	 * IF B not in A
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function NOT_LIKE ($param, $value) {
		#src/jotun/php/db/Clause.hx:62: characters 3-68
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} NOT LIKE ?",
			"value" => $value,
		]);
	}

	/**
	 * If A contains B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * 
	 * @return mixed
	 */
	public static function NOT_NULL ($param) {
		#src/jotun/php/db/Clause.hx:120: characters 3-79
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} IS NOT NULL",
			"value" => null,
			"skip" => true,
		]);
	}

	/**
	 * Group by entries by (A || B || ... || N)
	 * @param	conditions
	 * @return
	 * 
	 * @param mixed[]|\Array_hx $conditions
	 * 
	 * @return Clause
	 */
	public static function OR ($conditions) {
		#src/jotun/php/db/Clause.hx:33: characters 3-40
		return new Clause($conditions, " OR ");
	}

	/**
	 * IF A=B
	 * @param	param
	 * @param	value
	 * @return
	 * 
	 * @param string $param
	 * @param mixed $value
	 * 
	 * @return mixed
	 */
	public static function REGEXP ($param, $value) {
		#src/jotun/php/db/Clause.hx:162: characters 3-66
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}} REGEXP ?",
			"value" => $value,
		]);
	}

	/**
	 * IF A=1
	 * @param	param
	 * @return
	 * 
	 * @param string $param
	 * 
	 * @return mixed
	 */
	public static function TRUE ($param) {
		#src/jotun/php/db/Clause.hx:171: characters 3-58
		return new HxAnon([
			"param" => $param,
			"condition" => "{{p}}=?",
			"value" => true,
		]);
	}

	/**
	 *
	 * @param	conditions
	 * @param	joiner
	 * 
	 * @param mixed $conditions
	 * @param string $joiner
	 * 
	 * @return void
	 */
	public function __construct ($conditions, $joiner) {
		#src/jotun/php/db/Clause.hx:23: characters 3-19
		$this->_joiner = $joiner;
		#src/jotun/php/db/Clause.hx:24: characters 3-31
		$this->conditions = $conditions;
	}

	/**
	 *
	 * @return
	 * 
	 * @return string
	 */
	public function joiner () {
		#src/jotun/php/db/Clause.hx:287: characters 3-17
		return $this->_joiner;
	}
}

Boot::registerClass(Clause::class, 'jotun.php.db.Clause');
