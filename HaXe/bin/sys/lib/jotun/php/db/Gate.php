...
class Gate {
	...
	public function _log ($message) {
		#src/jotun/php/db/Gate.hx:228: lines 228-230
		Dice::Values($this->_onLog, function ($v) use (&$message) {
			#src/jotun/php/db/Gate.hx:229: characters 4-14
			$v($message);
		});
	}
	...
	public function getTableNames () {
		#src/jotun/php/db/Gate.hx:195: characters 3-28
		$r = new \Array_hx();
		#src/jotun/php/db/Gate.hx:196: lines 196-198
		Dice::Values($this->query("show tables")->execute()->result, function ($v) use (&$r) {
			#src/jotun/php/db/Gate.hx:197: characters 4-26
			Dice::Values($v, Boot::getInstanceClosure($r, 'push'));
		});
		#src/jotun/php/db/Gate.hx:199: characters 3-11
		return $r;
	}
	...
	public function getTables () {
		#src/jotun/php/db/Gate.hx:206: lines 206-212
		$_gthis = $this;
		#src/jotun/php/db/Gate.hx:207: characters 3-39
		$r = new HxAnon();
		#src/jotun/php/db/Gate.hx:208: lines 208-210
		Dice::Values($this->getTableNames(), function ($v) use (&$r, &$_gthis) {
			#src/jotun/php/db/Gate.hx:209: characters 4-22
			$value = $_gthis->table($v);
			\Reflect::setField($r, $v, $value);
		});
		#src/jotun/php/db/Gate.hx:211: characters 3-11
		return $r;
	}
	...
	public function ifTableExists ($table) {
		#src/jotun/php/db/Gate.hx:221: characters 10-17
		$a = $this->builder;
		#src/jotun/php/db/Gate.hx:222: characters 4-43
		$a1 = Clause::EQUAL("TABLE_SCHEMA", $this->_token->db);
		#src/jotun/php/db/Gate.hx:221: lines 221-224
		$a2 = $a->find("COUNT(*)", "INFORMATION_SCHEMA.TABLES", Clause::AND(\Array_hx::wrap([
			$a1,
			Clause::EQUAL("TABLE_NAME", $table),
		])), null, Limit::$ONE)->execute()->length();
		$aNeg = $a2 < 0;
		$bNeg = false;
		if ($aNeg !== $bNeg) {
			return $aNeg;
	...
	public function listen ($handler) {
		...
		if ($this->_onLog->indexOf($handler) === -1) {
			#src/jotun/php/db/Gate.hx:98: characters 4-24
			$_this = $this->_onLog;
	...
	public function open ($token) {
		...
		if (!$this->isOpen()) {
			...
			try {
				...
				#src/jotun/php/db/Gate.hx:116: characters 5-56
				$_this = $this->get_errors();
				#src/jotun/php/db/Gate.hx:116: characters 27-38
				$x = $e->getCode();
				#src/jotun/php/db/Gate.hx:116: characters 5-56
				$x1 = new Error($x, $e->getMessage());
	...
	public function schema ($table = null) {
		...
		#src/jotun/php/db/Gate.hx:156: characters 3-38
		$tableNames = new \Array_hx();
		#src/jotun/php/db/Gate.hx:157: lines 157-159
		Dice::Values($table, function ($v) use (&$tableNames) {
			#src/jotun/php/db/Gate.hx:158: characters 4-50
			$x = Clause::EQUAL("TABLE_NAME", $v);
			$tableNames->arr[$tableNames->length++] = $x;
		});
		#src/jotun/php/db/Gate.hx:160: characters 10-17
		$tmp = $this->builder;
		#src/jotun/php/db/Gate.hx:161: characters 4-43
		$tmp1 = Clause::EQUAL("TABLE_SCHEMA", $this->_token->db);
		#src/jotun/php/db/Gate.hx:160: lines 160-163
		return $tmp->find("*", "INFORMATION_SCHEMA.COLUMNS", Clause::AND(\Array_hx::wrap([
			$tmp1,
	...
	public function table ($table) {
		#src/jotun/php/db/Gate.hx:184: lines 184-186
		if (!\Reflect::hasField($this->_tables, $table)) {
			#src/jotun/php/db/Gate.hx:185: characters 4-50
			$this1 = $this->_tables;
			$value = new DataTable($table, $this);
			\Reflect::setField($this1, $table, $value);
		}
		#src/jotun/php/db/Gate.hx:187: characters 10-28
