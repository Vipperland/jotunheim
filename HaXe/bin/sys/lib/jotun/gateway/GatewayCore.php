...
class GatewayCore {
	...
	public static function init ($TGateway, $TOutput, $TDataAccess, $TInput, $TDomain, $options) {
		...
		#src+extras/gateway/jotun/gateway/GatewayCore.hx:43: lines 43-60
		if (!Flag::FTest($options, 16)) {
			...
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:48: characters 4-39
			$gateway->_output->setOptions($options);
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:50: characters 4-53
			$gateway->_database = new $TDataAccess->phpClassName();
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:52: characters 4-41
			$gateway->_database->setOptions($options);
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:54: characters 4-47
			$gateway->_domain = new $TDomain->phpClassName();
		} else {
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:58: characters 4-54
			$gateway->_output->error(1);
		}
		#src+extras/gateway/jotun/gateway/GatewayCore.hx:61: lines 61-63
		if (Flag::FTest($options, 32)) {
			#src+extras/gateway/jotun/gateway/GatewayCore.hx:62: characters 4-34
	...
	public function flush () {
		#src+extras/gateway/jotun/gateway/GatewayCore.hx:84: characters 3-18
		$this->_output->flush();
	}
}
