...
class ZoneCoreSession extends ZoneCoreObject {
	...
	public function save(id:String, device:String):Bool {
		if (_token == null && _uid == null){
			...
			_upd = _ctd;
			if (RunSQL(function(){
				return cast (_database, SessionDataAccess).user_session.add({
					_uid: id,
					_token: _token,
					_ip: _ip,
					_device: _device,
	...
	public function load(oauth:String):Bool {
		var current:ZoneCoreSession = RunSQL(function(){
			return cast (_database, SessionDataAccess).user_session.findOne("*", Clause.EQUAL('_token', oauth));
		});
		if (current != null){
			_uid = current._uid;
	...
	public function refresh():Void {
		if (_uid != null && _token != null){
			...
			RunSQL(function(){
				cast (_database, SessionDataAccess).user_session.updateOne({_upd:_upd}, Clause.EQUAL("_token", _token));
			});
		}
	}
	...
	public function drop():Void {
		RunSQL(function(){
			cast (_database, SessionDataAccess).user_session.deleteOne(Clause.EQUAL("_token", _token));
		});
	}
	...
	public function dropAll():Void {
		if (isValid()){
			RunSQL(function(){
				cast (_database, SessionDataAccess).user_session.delete(Clause.EQUAL("_uid", _uid));
			});
		}
	}
