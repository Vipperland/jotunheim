...
class Color {
	...
	static public var FLAT:Array<IColor> = [];
	
	static public var ALL:Array<IColor> = [];
	
	private static var _active:Bool = false;
	
	static public function rnd(?flat:Bool = false):IColor {
		if (ALL.length == 0) {
			Dice.All(COLORS, function(p:String, v:IColor) {
				v.name = p;
				ALL[ALL.length] = v;
			});
		}
		var v:Array<Dynamic> = (flat ? FLAT : ALL);
		return v[Std.random(v.length)];
	}
	
	
	static public function byName(name:String):IColor {
