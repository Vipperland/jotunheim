package gate.sirius.meta.core {
	...
	public class SiriusDecoder {
		...
		private static var _isSmartObjects:Boolean = true;
		
		
		public static function expand(domain:ApplicationDomain):void {
			var cnames:Vector.<String> = domain.getQualifiedDefinitionNames();
			for each (var c:String in cnames) {
				var def:String = c.split("::").join(".");
				if (domain.hasDefinition(def))
					SiriusDecoder.registerClass(domain.getDefinition(def));
			}
		}
		
		
		static public function traceClassNames():void {
			trace("[SiriusDecoder.classCollection {");
			var f:Array = [];
			for (var cname:String in _classList) {
			...
			f = f.sort();
			for each(cname in f) {
				trace("	Name: " + cname + ", namespace: " + getQualifiedClassName(_classList[cname]));
			}	
			trace("}]");
		}
		...
		public static function get smartObjects():Boolean {
			return _isSmartObjects;
		}
		
		
		public static function set smartObjects(value:Boolean):void {
			_isSmartObjects = value;
			if (_isSmartObjects) {
				OBJECT_CLASS = "Object";
			} else {
				OBJECT_CLASS = null;
		...
		public static function registerClass(... classList:Array):void {
			...
			for each (var LClass:*in classList) {
		...
		private static const SET:String = "/";
		...
		private static var OBJECT_CLASS:String = "Object";
				...
				if (index > 0) { // Set value
					...
					try {
						...
						if (paramValue.length == 0)
							...
							//trace(paramName, paramValue, typeof currentObject[paramName]);
		 ...
		 * @return
		 */
		public static function parse(data:String, onParseComplete:Function = null, target:SiriusObject = null):SiriusDecoder {
			
			if (!_preinit) {
				registerClass(Object, Array, SiriusObject, Sprite);
				_preinit = true;
			}
		...
		private var _content:SiriusObject;
		...
		public function SiriusDecoder(onParseComplete:Function = null, target:SiriusObject = null) {
			_content = target;
		...
		public function get content():SiriusObject {
			return _content;
		}
